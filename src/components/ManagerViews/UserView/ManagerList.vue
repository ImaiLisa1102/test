<template>
    <div class="list-area">
        ...
        <el-table :data="tableData" style="width: auto; font-size: 15px;">
            <el-table-column prop="man_username" label="用户名" width="180"></el-table-column>
            <el-table-column prop="man_password" label="密码" width="180"></el-table-column>
            <el-table-column prop="man_num" label="工号" width="140"></el-table-column>
            <el-table-column prop="man_email" label="联系方式" width="280"></el-table-column>
            <el-table-column prop="man_identity" label="真实姓名" width="180"></el-table-column>
...
const loadData = async () => {
    try {
        const response = await axios.get(`/api/list/manager?page=${currentPage.value}&size=${pageSize.value}`);
...
const handleDelete = (row) => {
    // 获取man_num
    const man_num = row.man_num;

    // 在控制台打印man_num，以便于调试
    console.log(`Deleting manager with man_num: ${man_num}`);

    // 发送一个DELETE请求到服务器，删除这一行的数据
    axios.delete(`/api/delete/manager/${man_num}`)
        .then(response => {
            // 删除成功后，重新加载数据
            loadData();
        })
