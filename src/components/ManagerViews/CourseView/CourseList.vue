    ...
    <div class="list-area">
        <el-row :gutter="20">
            <el-col :span="6" class="search-content" >
                <el-input placeholder="请输入课程号进行搜索" v-model="input" style="width: 200px;">
                </el-input>
        ...
        <el-table :data="tableData" style="width: auto; font-size: 15px;">
            ...
            <el-table-column prop="course_name" label="课程名" width="180"></el-table-column>1
            <el-table-column prop="course_term" label="课程学期" width="180"></el-table-column>
            <el-table-column prop="course_class" label="课程班级" width="180"></el-table-column>
            <el-table-column prop="course_grade" label="课程年级" width="180"></el-table-column>
...
const loadData = async () => {
    try {
        const response = await axios.get(`/api/list/course?page=${currentPage.value}&size=${pageSize.value}`);
...
const handleDelete = (row) => {
    ...
    // 在控制台打印man_num，以便于调试
    console.log(`Deleting course with course_id: ${course_id}`);

    // 发送一个DELETE请求到服务器，删除这一行的数据
    axios.delete(`/api/delete/course/${course_id}`)
        .then(response => {
            // 删除成功后，重新加载数据
            loadData();
        })
