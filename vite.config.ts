...
import * as path from 'path'; // 使用 ES6 的 import 语法
...
export default ({ command, mode }) => {
  ...
  return defineConfig({
    ...
    server: {
      ...
      //服务端代理设置
      proxy: {
        //如果访问地址以"/api"开头,则自动代理到变量VITE_ACADEMIC_CONTEXT_PATH所表示的
		//服务端地址http://localhost:9999/academic
        '/api': {
          target: env.VITE_ACADEMIC_CONTEXT_PATH,
          changeOrigin: true,
